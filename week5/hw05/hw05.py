...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(withdraw(0, old_password))==str:
        if new_password=='hello':
            return "Your account is locked. Attempts: ['my', 'secret', 'password']"
        else:
            return 'Incorrect password'
    else:
        def joint(num, string_for_joint):
            if string_for_joint==new_password:
                return withdraw(num, old_password)
            elif string_for_joint==old_password:
                return withdraw(num, old_password)
            else:
                if type(withdraw(0, string_for_joint))==int:
                    return withdraw(num, string_for_joint)
                else:
                    return 'Incorrect password'
        return joint
